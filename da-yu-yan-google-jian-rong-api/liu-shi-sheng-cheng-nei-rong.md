...
Google AI Studio API Key（认证方式1）

### Body 参数 `application/json` 必填

#### contents

`array[object (Gemini Chat 请求体-Content)]`，必需
...
`array[object (Gemini Chat 请求体-Part)]`，必需
...
`object(Gemini Chat 请求体-Content)`，可选
...
`object(Gemini Chat 请求体-ChatGenerationConfig)`，可选
...
`object(Gemini Chat 请求体-ThinkingConfig)`，可选

思考配置。启用后，模型会展示内部推理过程

**safetySettings**

`array[object (Gemini Chat 请求体-ChatSafetySettings)]`，可选
...
`HARM_CATEGORY_HARASSMENTHARM_CATEGORY_HATE_SPEECHHARM_CATEGORY_SEXUALLY_EXPLICITHARM_CATEGORY_DANGEROUS_CONTENTHARM_CATEGORY_CIVIC_INTEGRITY`
...
`array[object (Gemini Chat 请求体-ChatTool)]`，可选
...
`object(Gemini Chat 请求体-Config)`，可选

工具配置，用于控制工具的调用行为（如强制调用、允许的函数列表等）

**functionCallingConfig**

`object(Gemini Chat 请求体-FunctionCallingConfig)`，可选
...
url = "https://router.shengsuanyun.com/api/v1/models/:streamGenerateContent?alt=sse&key=<api-key>"
...
fetch("https://router.shengsuanyun.com/api/v1/models/:streamGenerateContent?alt=sse&key=<api-key>", requestOptions)
...
```shellscript
curl --location 'https://router.shengsuanyun.com/api/v1/models/:streamGenerateContent?alt=sse&key=%3Capi-key%3E' \
...
if(curl) {
   ...
   curl_easy_setopt(curl, CURLOPT_URL, "https://router.shengsuanyun.com/api/v1/models/:streamGenerateContent?alt=sse&key=%3Capi-key%3E");
